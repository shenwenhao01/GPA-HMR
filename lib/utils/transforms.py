...
def inverse_affine_transform(transformed_pt, t):
    # Extract the 2x2 linear transformation part and the translation part
    linear_part = t[:, :2]  # 2x2 matrix
    translation_part = t[:, 2]  # 2-element vector
    
    # Compute the inverse of the linear transformation part
    linear_part_inv = np.linalg.inv(linear_part)
    
    # Compute the original point using the inverse transformation
    transformed_pt_adjusted = transformed_pt - translation_part
    original_pt = np.dot(linear_part_inv, transformed_pt_adjusted)
    
    return original_pt

def inverse_affine_batch(pts_prime, T):
    """
    对一批 2D 关键点执行仿射逆变换。
    参数:
        pts_prime: [B, N, 2] 变换后的关键点坐标
        T: [B, 2, 3] 每个样本的前向仿射矩阵
    返回:
        pts: [B, N, 2] 逆变换后的关键点坐标
    """
    A = T[:, :, :2]               # (B, 2, 2)
    t = T[:, :, 2]                # (B, 2)
    
    # 计算 A 的逆
    A_inv = np.linalg.inv(A)      # (B, 2, 2)
    
    # pts_prime - t（广播到 (B, N, 2)）
    pts_shifted = pts_prime - t[:, None, :]  # (B, N, 2)
    
    # 计算逆变换： (pts_prime - t) @ A_inv^T
    pts = np.einsum('bij,bnj->bni', A_inv, pts_shifted)
    return pts
