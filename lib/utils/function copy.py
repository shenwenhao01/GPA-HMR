...
from models.loss import SMPL_LOSS, SCORE_LOSS, DPO_SMPL_LOSS
...
def get_dataloader(config, is_train = True):
    if is_train:
        ...
        dataset3 = hp3d(
                cfg=config,
                ann_file=config.dataset.set_list[2].test_set,
                root = config.dataset.set_list[2].root,
                train=False)
        datasets = {'h36m': dataset1, '3dpw':dataset2, '3dhp': dataset3}
    dataloaders = {}
    samplers = {}
...
def get_model(config, is_train = True, resume = False, resume_path = None):
    ...
    if resume:
        ...
        if not config.training.get('dpo', False):
