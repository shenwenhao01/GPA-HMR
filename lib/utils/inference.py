...
def output2original_scale(meta, output, vis=False,start=0):
    ...
    for i, img_path in enumerate(img_paths):
        ...
        if vis:
           cv2.imwrite(f'origin_det_{i}.jpg', img)
...
def visualize(results,draw_num,save_path,detection_all=None,input_type='image', obj_save_path=None, smpl_faces=None):
    ...
    if input_type=='video':
        ...
        for mesh_idx in tqdm(range(pred_mesh.shape[0])):
            ...
            for i in range(draw_num):
                ...
                rgb, depth = render_mesh_fast(ori_img_height, ori_img_width, [pred_mesh_T/1000], smpl.faces, {'focal': focal_l[mesh_idx], 'princpt': center_pt[mesh_idx]})
                side_rgb, side_depth = render_mesh_fast_side_view(ori_img_height, ori_img_width, 
                                                                  [pred_mesh_T/1000], smpl.faces, {'focal': focal_l[mesh_idx], 'princpt': center_pt[mesh_idx]},
                                                                  yaw_deg=180)
                ...
                rendered_side_img = side_rgb
                save_path_img = osp.join(save_path, img_name[mesh_idx], str(idx_list[mesh_idx]), f"idx{idx_list[mesh_idx]} hypo{i}_side.jpg")
                cv2.imwrite(save_path_img, rendered_side_img.astype(np.uint8)[...,::-1])
                if obj_save_path is not None:
                    save_path_obj = osp.join(obj_save_path, img_name[mesh_idx], str(idx_list[mesh_idx]), f"idx{idx_list[mesh_idx]}_hypo{i}.obj")
                    os.makedirs(osp.join(obj_save_path, img_name[mesh_idx], str(idx_list[mesh_idx])), exist_ok=True)
                    write_obj(save_path_obj, pred_mesh[mesh_idx, i], smpl_faces)
