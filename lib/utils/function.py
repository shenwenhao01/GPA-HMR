...
from models.loss import SMPL_LOSS, SCORE_LOSS, DPO_SMPL_LOSS, KTO_SMPL_LOSS, GRPO_SMPL_LOSS
...
def get_dataloader(config, is_train = True):
    if is_train:
        if config.training.get('dpo', False):
            ...
            datasets = {}
            datasets['grpo'] = GRPODataset(cfg=config, train=True)
        ...
        #         data_split='test')
        dataset4 = InstaVariety(
                cfg=config,
                transform=torchvision.transforms.ToTensor(),
                data_split='test')
        # datasets = {'h36m': dataset1, '3dpw':dataset2, 'hi4d': dataset3, 'instavariety': dataset4}
        datasets = {'h36m': dataset1, '3dpw':dataset2, 'instavariety': dataset4}
    ...
    for key, dataset in datasets.items():
        ...
        if is_train:
            # sampler = torch.utils.data.distributed.DistributedSampler(dataset, shuffle=False)             # [BUG] no shuffling
...
def get_model(config, is_train = True, resume = False, resume_path = None):
    ...
    if is_train:
        ...
        if config.training.get('grpo', False):
            loss = GRPO_SMPL_LOSS(config).to(config.device)
    ...
    if resume:
        ...
        if not (config.training.get('dpo', False) or config.training.get('kto', False) or config.training.get('grpo', False)):
...
def get_model_score(config, is_train = True, resume = False, resume_path = None):
    ...
    if is_train:
        model_score.train()
        # BUG disbale model_score_cond training
