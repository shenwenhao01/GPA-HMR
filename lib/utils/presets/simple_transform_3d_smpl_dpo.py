...
class SimpleTransform3DSMPL_DPO(object):
    ...
    def __call__(self, src, label, label_dpo):
        if self.two_d:
            ...
            dpo_trans = label_dpo['trans']
            dpo_trans_inv = label_dpo['trans_inv']
            l_joints_29_uvd = label_dpo['l_joints']
            w_joints_29_uvd = label_dpo['w_joints']
            l_joints_29_uvd = np.concatenate((l_joints_29_uvd[..., np.newaxis], joints_vis_29[..., np.newaxis]), axis=-1)
            w_joints_29_uvd = np.concatenate((w_joints_29_uvd[..., np.newaxis], joints_vis_29[..., np.newaxis]), axis=-1)
            for i in range(29):
                if l_joints_29_uvd[i, 0, 1] > 0.0:
                    l_joints_29_uvd[i, 0:2, 0] = inverse_affine_transform(l_joints_29_uvd[i, 0:2, 0], dpo_trans)
                    # l_joints_29_uvd[i, 0:2, 0] = affine_transform(l_joints_29_uvd[i, 0:2, 0], dpo_trans_inv)
                # joints_2d_uvd[i,:,0] = inverse_affine_transform(joints_2d_uvd[i, :, 0], trans)
                if w_joints_29_uvd[i, 0, 1] > 0.0:
                    w_joints_29_uvd[i, 0:2, 0] = inverse_affine_transform(w_joints_29_uvd[i, 0:2, 0], dpo_trans)
                    # w_joints_29_uvd[i, 0:2, 0] = affine_transform(w_joints_29_uvd[i, 0:2, 0], dpo_trans_inv)
            ...
            l_twist_phi = label_dpo['l_twist'].copy()
            w_twist_phi = label_dpo['w_twist'].copy()
            ...
            if random.random() > 0.5 and self._train and self._flip:
                ...
                l_joints_29_uvd = flip_joints_3d(l_joints_29_uvd, imgwidth, self._joint_pairs_29)
                w_joints_29_uvd = flip_joints_3d(w_joints_29_uvd, imgwidth, self._joint_pairs_29)
                ...
                l_twist_phi, _ = flip_twist(l_twist_phi, twist_weight, self._joint_pairs_24)
                w_twist_phi, _ = flip_twist(w_twist_phi, twist_weight, self._joint_pairs_24)
                center[0] = imgwidth - center[0] - 1
            ...
            for i in range(29):
                ...
                if l_joints_29_uvd[i, 0, 1] > 0.0:
                    l_joints_29_uvd[i, 0:2, 0] = affine_transform(l_joints_29_uvd[i, 0:2, 0], trans)
                if w_joints_29_uvd[i, 0, 1] > 0.0:
                    w_joints_29_uvd[i, 0:2, 0] = affine_transform(w_joints_29_uvd[i, 0:2, 0], trans)
        ...
        if self.two_d:
            ...
            output = {
                ...
                'l_joints_uvd_29':torch.from_numpy(l_joints_29_uvd[:,:,0]).float(),
                'w_joints_uvd_29':torch.from_numpy(w_joints_29_uvd[:,:,0]).float(),
                ...
                'target_l_twist': torch.from_numpy(l_twist_phi).float(),
                'target_w_twist': torch.from_numpy(w_twist_phi).float(),
