...
from ..transforms import (addDPG, affine_transform, inverse_affine_transform, flip_joints_3d, flip_thetas, flip_xyz_joints_3d,
                          get_affine_transform, im_to_torch, batch_rodrigues_numpy, inverse_affine_batch,
...
class SimpleTransform3DSMPL_GRPO(object):
    ...
    def __call__(self, src, label, label_grpo):
        if self.two_d:
            ...
            if imgwidth != src.shape[1] or imght != src.shape[0]:
                src = cv2.resize(src, (2048, 2048))
            ...
            multi_n = label_grpo['multi_n']
            dpo_trans = label_grpo['trans']
            dpo_trans_inv = label_grpo['trans_inv']
            pred_joints_29_vis = label_grpo['pred_joints']
            pred_joints_29_uvd_origin = label_grpo['pred_joints_uvd_29']
            gt_joints_29_uvd = label_grpo['gt_joints']
            gt_joints_29_uvd = np.concatenate((gt_joints_29_uvd[..., np.newaxis], joints_vis_29[..., np.newaxis]), axis=-1)
            # pred_joints_29_uvd_ = np.concatenate((pred_joints_29_vis[..., np.newaxis], joints_vis_29[np.newaxis, ..., np.newaxis].repeat(multi_n, axis=0)), axis=-1)
            # for b in range(pred_joints_29_uvd_.shape[0]):
            #     for i in range(29):
            #         if pred_joints_29_uvd_[b, i, 0, 1] > 0.0:
            #             pred_joints_29_uvd_[b, i, 0:2, 0] = inverse_affine_transform(pred_joints_29_uvd_[b, i, 0:2, 0], dpo_trans)
            #             # pred_joints_29_uvd[b, i, 0:2, 0] = affine_transform(pred_joints_29_uvd[b, i, 0:2, 0], dpo_trans_inv)
            #         # joints_2d_uvd[i,:,0] = inverse_affine_transform(joints_2d_uvd[i, :, 0], trans)
            
            pred_joints_29_uvd = np.concatenate((pred_joints_29_uvd_origin[..., np.newaxis], joints_vis_29[np.newaxis, ..., np.newaxis].repeat(multi_n, axis=0)), axis=-1)
            for i in range(29):    
                if gt_joints_29_uvd[i, 0, 1] > 0.0:
                    gt_joints_29_uvd[i, 0:2, 0] = inverse_affine_transform(gt_joints_29_uvd[i, 0:2, 0], dpo_trans)
                    # gt_joints_29_uvd[i, 0:2, 0] = affine_transform(gt_joints_29_uvd[i, 0:2, 0], dpo_trans_inv)
            ...
            pred_twist_phi = label_grpo['pred_twist'].copy()
            gt_twist_phi = label_grpo['gt_twist'].copy()
            ...
            if random.random() > 0.5 and self._train and self._flip:
                ...
                for b in range(pred_joints_29_uvd.shape[0]):
                    pred_joints_29_uvd[b, ...] = flip_joints_3d(pred_joints_29_uvd[b], imgwidth, self._joint_pairs_29)
                    pred_twist_phi[b, ...], _ = flip_twist(pred_twist_phi[b], twist_weight, self._joint_pairs_24)
                gt_joints_29_uvd = flip_joints_3d(gt_joints_29_uvd, imgwidth, self._joint_pairs_29)
                ...
                gt_twist_phi, _ = flip_twist(gt_twist_phi, twist_weight, self._joint_pairs_24)
                center[0] = imgwidth - center[0] - 1
            ...
            for i in range(29):
                ...
                for b in range(multi_n):
                    if pred_joints_29_uvd[b, i, 0, 1] > 0.0:
                        pred_joints_29_uvd[b, i, 0:2, 0] = affine_transform(pred_joints_29_uvd[b, i, 0:2, 0], trans)
                if gt_joints_29_uvd[i, 0, 1] > 0.0:
                    gt_joints_29_uvd[i, 0:2, 0] = affine_transform(gt_joints_29_uvd[i, 0:2, 0], trans)
        ...
        if self.two_d:
            ...
            output = {
                ...
                # w/l joints
                'joints_uvd_29':torch.from_numpy(joints_29_uvd[:, :, 0]).float(),             # 29, 3
                'pred_joints_uvd_29':torch.from_numpy(pred_joints_29_uvd[:, :, :, 0]).float(),   # multi_n, 29, 3
                'gt_joints_uvd_29':torch.from_numpy(gt_joints_29_uvd[:, :, 0]).float(),       # 29,3
                # GT
                ...
                'pred_twist': torch.from_numpy(pred_twist_phi).float(),             # multi_n, 23, 2
                'gt_twist': torch.from_numpy(gt_twist_phi).float(),
