...
def denormalize_pose_cuda(pose, mean_and_std=None,res_w_h=None, which='zero_center',scale = None,two_d = False):
    ...
    if which == 'zero_center':
        ...
        if not mean_and_std:
            mean_and_std = 1
        pose = pose / 2                                 # [-50, 50] -> [-0.5 * 50, 0.5*50]
...
def uvd_to_cam(uvd_jts, trans_inv, intrinsic_param, joint_root, depth_factor, return_relative=True):
    ...
    uv_jts = torch.matmul(trans_inv.unsqueeze(1), uv_homo_jts.unsqueeze(-1))            #[29, 2, 1] joints in original image space
...
def process_output(output_m, labels, smpl, process=False):
    ...
    joints = output_m['pred_joints'].clone()                        # uvd joints: camera space depth, image space uv
    num_joints = output_m['pred_joints'].size(1)

    # uvd_root: predicted 2D joints in original img space from the network
    # pred_joints: recovered 3D joints in camera space with root joint depth from labels['joint_root'] 如果数据集里面有这个gt 不然就是0
    ...
    output = smpl.hybrik(
            pose_skeleton=pred_final.clone().type(torch.float32) * 2,       # joints in cam space
