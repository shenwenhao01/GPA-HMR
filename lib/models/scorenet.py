...
class ScoreNet(nn.Module):
    def __init__(self, cfg, neighbour_matrix=None):
        ...
        mask_k = 3 if self.num_twists != 0 else 2
        for i in range(self.num_blocks):
            mask_i = np.linalg.matrix_power(neighbour_matrix[mask_k], self.atten_knn[i])
    ...
    def forward(self, xinj, ctx=None):
        '''
        xinj: [bs * num_hypoes, 133]
        ctx['local']: [bs, 256, 64, 64]
        ctx['global']: [bs, 512, 8, 8]
        '''
        bs = ctx['global'].shape[0]
        assert xinj.shape[0] % bs == 0
        multi_n = xinj.shape[0] // bs
        whole = bs * multi_n
        xint = xinj[:, 3*self.joints:].clone()              # [bs * multi_n, 46]
        xinj = xinj[:, :3*self.joints].clone()              # [bs * multi_n, 87]
        
        ctx['local'] = self.get_local_feature(xinj, ctx['local'])              # [bs, multi_n, 52*256(13312)]
        ...
        emb_ctx = positionalencoding2d(self.ch, self.emb_h, self.emb_w).unsqueeze(dim=0).repeat(ctx['global'].size()[0],1,1,1).cuda()       # [bs, 512, 8, 8]
        emb_ctx = torch.flatten(emb_ctx,start_dim=2, end_dim=3).transpose(2,1)                                                              # [bs, 64, 512]
        ctx['global'] = torch.flatten(ctx['global'], start_dim=2, end_dim=3).transpose(2, 1).to(xinj.device)                                # [bs, 512, 8, 8]
        emb = get_timestep_embedding(torch.arange(0, self.num_item), self.ch).cuda().view(1, self.num_item, self.ch).to(xinj.device)        # [1, 52, 512]
        emb_multi = get_timestep_embedding(torch.arange(0, self.num_item), self.ch).cuda().view(1, -1, self.ch)                             # [1, 52, 512]
        emb_multi = emb_multi.unsqueeze(1).repeat(1, multi_n, 1, 1).view(1,-1,self.ch)                                                      # [1, 52 * multi_n, 512]

        # joints
        x0 = self.linear_start_j(xinj)                                                                              # [bs * multi_n, 7424]
        x0 = self.bn_start_j(x0)
        x0 = self.activation_start_j(x0)
        x0 = self.dropout_start_j(x0)
        x0 = x0.contiguous().view(-1, self.joints, self.joint_ch)                                                   # [bs * multi_n, 29, 256]
        # twist
        # BUG
        if self.num_twists != 0:
            x1 = self.linear_start_t(xint)                                                                              # [bs * multi_n, 5888]
            x1 = self.bn_start_t(x1)
            x1 = self.activation_start_t(x1)
            x1 = self.dropout_start_t(x1)
            x1 =  x1.contiguous().view(-1, self.num_twists,self.joint_ch)                                               # [bs * multi_n, 23, 256]
        else:
            x1 = torch.zeros_like(x0)[:, 0:0]
        # combine
        x = torch.cat([x0, x1], dim=1).view(-1, self.num_item, self.joint_ch)                                       # [bs * multi_n, 52, 256]
        ctx['local'] = ctx['local'].view(-1, self.num_item, self.joint_ch).to(xinj.device)      # pixel-aligned sample          # [bs * multi_n, 52, 256]
        x = torch.cat([x, ctx['local']], dim=-1).view(-1, self.num_item, self.ch)                                   # [bs * multi_n, 52, 512]
        ...
        # final layer
        x = x.view(-1, self.num_item*self.ch)                                                                       # [bs * multi_n, 52*512(26624)]
