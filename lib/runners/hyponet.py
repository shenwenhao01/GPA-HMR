...
class HypoNetTrainer(object):
    ...
    def train(self):
        ...
        if config.training.get('dpo', False) or config.training.get('kto', False) or config.training.get('grpo', False):
        ...
        elif config.training.get('grpo', False):
            train_loader = train_loaders['grpo']
            train_dataset = train_datasets['grpo']
            train_sampler = train_samplers['grpo']
        else:
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i,  (inps, labels, img_ids, bboxes) in enumerate(train_loader1):
                ...
                if config.training.get('dpo', False):
                    ...
                    l_joints = labels['l_joints_uvd_29'][:,:self.num_joints].float().to(self.device).clone()            # B, 29, 3
                    ...
                    x_wl = torch.cat([w_x, l_x], dim=0)                                                                 # 2*B, 87
                    ...
                    B, G, _, _ = labels['pred_twist'].shape

                    pred_twist = labels['pred_twist'].clone().float().to(self.device).view(B, G, -1) * self.config.diffusion.scale
                    pred_joints = labels['pred_joints_uvd_29'][:, :,:self.num_joints].float().to(self.device).clone()
                    # pred_joint_2d = labels['pred_joints_uvd_29'][:, :,:self.num_joints,:2].float().to(self.device).clone()
                    pred_joints = pred_joints.view(B*G, self.num_joints, 3)
                    # pred_joint_2d = pred_joint_2d.view(B*M, self.num_joints, 2)
                    pred_twist = pred_twist.view(B*G, -1)                           # M, M, M, ...
                    # pred_joint_2d = normalize_pose_cuda(pose = pred_joint_2d.clone(), which='scale_t', scale=scale_2d).to(self.device)
                    pred_x = normalize_pose_cuda(pose = pred_joints, which=config.hyponet.norm, scale=scale, mean_and_std=self.config.diffusion.scale).to(self.device)        # bs, 87
                    
                    # ctx['global']: bs, 512, 8, 8 ; ctx['local']: bs, 40, 256
                    ctx, pred_shape, pred_2d = model_cond(input)
                    # ctx['global'] = ctx['global'].repeat(M, 1, 1, 1)
                    # ctx['local'] = ctx['local'].repeat(M, 1, 1)
                    ctx['global'] = ctx['global'].unsqueeze(1).repeat(1, G, 1, 1, 1)   # (B, M, 512, 8, 8)
                    ctx['global'] = ctx['global'].view(B * G, 512, 8, 8)
                    ctx['local'] = ctx['local'].unsqueeze(1).repeat(1, G, 1, 1)
                    ctx['local'] = ctx['local'].view(B * G, self.num_twists+self.num_joints, 256)
                    ctx_ref, pred_shape_ref, pred_2d_ref = ref_model_cond(input)
                    # ctx_ref['global'] = ctx_ref['global'].repeat(M, 1, 1, 1)
                    # ctx_ref['local'] = ctx_ref['local'].repeat(M, 1, 1)
                    ctx_ref['global'] = ctx_ref['global'].unsqueeze(1).repeat(1, G, 1, 1, 1)   # (B, M, 512, 8, 8)
                    ctx_ref['global'] = ctx_ref['global'].view(B * G, 512, 8, 8)
                    ctx_ref['local'] = ctx_ref['local'].unsqueeze(1).repeat(1, G, 1, 1)
                    ctx_ref['local'] = ctx_ref['local'].view(B * G, self.num_twists+self.num_joints, 256)


                    # t = torch.randint(low=0, high=self.num_timesteps, size=(n // 2 + 1,)).to(self.device)
                    # t = torch.cat([t, self.num_timesteps - t - 1], dim=0)[:n]
                    # BUG add timestep clip strategy
                    t = torch.randint(low=500, high=self.num_timesteps, size=(n,)).to(self.device)
                    # NOTE Use the same noise for one group
                    t = t.unsqueeze(1).repeat(1, G).view(B * G)                     # M,M,M,...
                    
                    # NOTE keep the same for each Group
                    ej = torch.randn_like(x).to(self.device).unsqueeze(1).repeat(1, G, 1).view(B * G, -1)      # B*M
                    et = torch.randn_like(twist).to(self.device).unsqueeze(1).repeat(1, G, 1).view(B * G, -1)
                    gt = {'noise_j': ej, 'noise_t': et, 'joint_2d': joint_2d.view(-1, self.num_joints, 2)}
                    a = self.alpha.clone().to(self.device).index_select(0, t).view(-1, 1)
                    
                    # Forward Process
                    xinj = pred_x * a.sqrt() + ej * (1.0 - a).sqrt()
                    xint = pred_twist * a.sqrt() + et * (1.0 - a).sqrt()
                    
                    # Reverse Process
                    output_list = model(xinj=xinj, xint=xint, t=t.float(), ctx=ctx)
                    output = {'pred_shape': pred_shape, 'joint_2d': pred_2d.view(-1, self.num_joints, 2), 'noise_j': output_list[0], 'noise_t': output_list[1]}
                    loss_origin, loss_list = loss_smpl(output, gt, labels, G)
                    model_loss_joint = loss_list['loss_joint']
                    model_loss_twist = loss_list['loss_twist']
                    ...
                    with torch.no_grad(): # Get the reference policy (unet) prediction
                        ref_output_list = ref_model(xinj=xinj, xint=xint, t=t.float(), ctx=ctx_ref)
                        ref_output = {'pred_shape': pred_shape_ref, 'joint_2d': pred_2d_ref.view(-1, self.num_joints, 2), 'noise_j': ref_output_list[0].detach(), 'noise_t': ref_output_list[1].detach()}
                        ref_loss, ref_loss_list = loss_smpl(ref_output, gt, labels, G)
                        ref_loss_joint = ref_loss_list['loss_joint']
                        ref_loss_twist = ref_loss_list['loss_twist']
                    
                    # DRPO loss
                    delta_loss_joint = model_loss_joint - ref_loss_joint
                    delta_loss_twist = model_loss_twist - ref_loss_twist

                    sum_pos_joint = (labels['grpo_w'] * labels['grpo_idx_pos'] * delta_loss_joint).sum(dim=1)
                    sum_neg_joint = (labels['grpo_w'] * labels['grpo_idx_neg'] * delta_loss_joint).sum(dim=1)
                    sum_pos_twist = (labels['grpo_w'] * labels['grpo_idx_pos'] * delta_loss_twist).sum(dim=1)
                    sum_neg_twist = (labels['grpo_w'] * labels['grpo_idx_neg'] * delta_loss_twist).sum(dim=1)
                    
                    scale_term = -1.0 * args.beta_grpo
                    joint_inside_term = scale_term * (sum_pos_joint - sum_neg_joint)
                    grpo_joint_loss = -1 * F.logsigmoid(joint_inside_term).mean()

                    twist_inside_term = scale_term * (sum_pos_twist - sum_neg_twist)
                    # twist_implicit_acc = (twist_inside_term > 0).sum().float() / twist_inside_term.size(0)
                    grpo_twist_loss = -1 * F.logsigmoid(twist_inside_term).mean()

                    loss = (grpo_joint_loss + grpo_twist_loss) * 0.1 + loss_origin              # BUG SOTA add origin loss
                    # loss = (grpo_joint_loss + grpo_twist_loss) * 0.2
                    # print(loss)
                else:
                    joints = labels['joints_uvd_29'][:,:self.num_joints].float().to(self.device)
                    twist = labels['target_twist'].float().to(self.device).view(n,-1) * self.config.diffusion.scale
                ...
                if step % self.config.training.loss_freq == 0 and self.rank ==0:
                    if config.training.get('dpo', False):
                        ...
                        logging.info(f"epoch:{epoch}, step: {step}, loss: {loss.item()}, batch size: {n} loss_origin: {loss_origin.item()}, grpo_joint_loss: {grpo_joint_loss}, grpo_twist_loss: {grpo_twist_loss}")
                    else:
                        logging.info(f"epoch:{epoch}, step: {step}, loss: {loss.item()}, batch size: {n}")
                step += 1
