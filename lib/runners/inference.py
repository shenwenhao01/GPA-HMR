        ...
        with torch.no_grad():
            ...
            for i, (inps, labels, img_ids) in enumerate(tqdm(dataloader, ncols=100)):
                ...
                if not self.config.inference.input_type == 'video' and self.config.inference.render:
                    visualize(results, config.inference.draw_num,save_path= os.path.join(self.config.inference.out_dir,'image'),detection_all=state['detection_all'],input_type=self.config.inference.input_type, obj_save_path=os.path.join(self.config.inference.out_dir,'mesh'), smpl_faces=self.smpl.faces.copy())
                
                # BUG abandon integrated into visualize function above
                # if self.config.inference.write_obj:
                #     path_s = os.path.join(self.config.inference.out_dir,'mesh')
                #     for k in range(config.inference.obj_num):
                #         for j in range(pred_xyz_jts_17.shape[0]):
                #             path_i = os.path.join(path_s, labels['img_name'][j])
