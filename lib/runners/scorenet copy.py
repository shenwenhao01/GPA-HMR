...
class ScorenetTrainer(object):
    ...
    def sample(self, state, construct_dpo=False):
        ...
        with torch.no_grad():
            ...
            for i, (inps, labels, img_ids, bboxes, img_path) in enumerate(tqdm(dataloader, ncols=100)):
                ...
                for k, _ in labels.items():
                    labels[k] = labels[k].to(self.device)
                input = inps.float().to(self.device)
                save_trans_inv = labels['trans_inv'].tolist()
                ...
                if construct_dpo:
                    save_pred_joints = output['pred_joints'].tolist()
                    save_pred_twist = output['pred_twist'].tolist()
                    save_joints_uvd_29 = labels['joints_uvd_29'].tolist()
                    save_w_twist = labels['target_twist'].tolist()
                    save_trans = labels['trans'].tolist()
                    for idx in range(n):
        ...
        if construct_dpo:
            save_path = f'data/dpo/annotations/{dataset.__class__.__name__}_train_interval_{config.sampling.interval}_multihypo_{config.sampling.multihypo_n}.json'
