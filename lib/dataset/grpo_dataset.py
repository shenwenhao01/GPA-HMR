...
from dataset.h36m_dpo import H36MDataset_DPO
from dataset.pw3d_dpo import PW3D_DPO
from dataset.InstaVariety_grpo import InstaVariety_GRPO
from dataset.h36m_grpo import H36MDataset_GRPO
from dataset.pw3d_grpo import PW3D_GRPO
...
class GRPODataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self.db0 = H36MDataset_GRPO(
            cfg=cfg,
            dpo_ann_file=cfg.dataset.dpo_set_list[0].train_set,
            root = cfg.dataset.set_list[0].root,
            dpo_root=cfg.dataset.dpo_set_list[0].root,
        ...
        self.db3 = PW3D_GRPO(
            ...
            dpo_ann_file=cfg.dataset.dpo_set_list[3].train_set,
            root = cfg.dataset.set_list[3].root,
            dpo_root=cfg.dataset.dpo_set_list[3].root,
            train=True)
        self.db4 = MPII(
            cfg=cfg,
        ...
        self.db5 = UP3D(
            ...
            train=True)
        self.db6 = InstaVariety_GRPO(
            cfg=cfg,
            transform=torchvision.transforms.ToTensor(),
            data_split='train',
            dpo_ann_file=cfg.dataset.dpo_set_list[6].train_set,
            dpo_root=cfg.dataset.dpo_set_list[6].root)
        ...
        if cfg.training.get('dpo', False) or cfg.training.get('grpo', False):                  # for DPO training set length control
            dpo_set_list = []
            for i, partition in enumerate(self.partition):
                if partition > 0:
                    dpo_set_list.append(i)
            self._dpo_subset = [self._subsets[i] for i in dpo_set_list]
            self._dpo_subset_size = [len(item) for item in self._dpo_subset]
            self.tot_size = cfg.dataset.tot_size                  # BUG fix tot_size
            logging.info(f"DPO/GRPO Dataset Total Size: {self.tot_size}")
