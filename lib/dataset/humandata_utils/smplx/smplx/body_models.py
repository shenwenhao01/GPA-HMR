...
class SMPLX(SMPLH):
    ...
    def get_tpose(self, betas=None, gender=None):
        kwargs = {}
        if betas is None:
            betas = torch.zeros(1, 10).to(self.J_regressor.device)
        kwargs['betas'] = betas
        ...
        J_template = torch.einsum('ji,ik->jk', [self.J_regressor[:22], self.v_template])
        J_dirs = torch.einsum('ji,ikl->jkl', [self.J_regressor[:22], self.shapedirs])
        ...
        for gi, g in enumerate(['male', 'female', 'neutral']):
            ...
            J = J_template.unsqueeze(0) + blend_shapes(gender_kwargs['betas'], J_dirs)
