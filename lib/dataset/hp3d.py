...
class HP3D(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.exists(img_path):
            ...
            new_path = '/'.join(path_parts[:6]) + '/imageFrames/' + f'video_{path_parts[-2][-1]}' + f'/frame_{path_parts[-1][-10:]}'
            if not os.path.exists(new_path):
                new_path = '/'.join(path_parts[:6]) + '/imageSequence/' + f'img_{path_parts[-2][-1]}_{path_parts[-1][-10:]}'
            img_path = new_path
