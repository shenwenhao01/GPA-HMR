...
from utils.presets import (SimpleTransform3DSMPL_DPO, SimpleTransform3DSMPL_GRPO, 
                                  SimpleTransform3DSMPLCam)
from tqdm import tqdm
from utils.transforms import flip_xyz_joints_3d


class H36MDataset_GRPO(data.Dataset):
    ...
    def __init__(self,
        ...
        self.grpo_error_weights = cfg.training.grpo_error_weights
    ...
    def __getitem__(self, idx):
        ...
        img_path = self.db['img_path'][img_id]          # 必须要额外存一个img_path里面的绝对id

        # load ground truth, including bbox, keypoints, image size
        label = {}
        for k in self.db.keys():
            label[k] = self.db[k][img_id].copy()
        label_dpo = {}
        ...
        label_dpo['mpjpe'] = label_dpo['mpjpe'][group_idx]
        label_dpo['pampjpe'] = label_dpo['pampjpe'][group_idx]
        label_dpo['pve'] = label_dpo['pve'][group_idx]

        grpo_dict = self.dgpo_advantage_from_errors(label_dpo['mpjpe'], label_dpo['pampjpe'], label_dpo['pve'], alpha=self.grpo_error_weights)
    ...
    def dgpo_advantage_from_errors(
        self, mpjpe: np.ndarray, pampjpe: np.ndarray, pve: np.ndarray,
        alpha = (1.0, 1.0, 1.0),
        eps: float = 1e-8,
    ):
        """
        DGPO Eq.(12)-(14) faithful implementation.

        Args:
            mpjpe, pampjpe, pve: 1D arrays (length = group size G). These are *errors* (lower is better).
            alpha: weights to fuse errors into a single scalar reward r_i = -(a1*mpjpe + a2*pampjpe + a3*pve).
                Choose alpha to balance scales if needed. Keeping it explicit is faithful to the paper.
            eps: small constant to avoid division by zero in std.

        Returns:
            A dict with:
            - r:       (G,) scalar rewards
            - A:       (G,) advantages (z-score within the group)
            - w:       (G,) weights w_i = |A_i|
            - idx_pos: indices of G^+ (A_i > 0)
            - idx_neg: indices of G^- (A_i <= 0)
            - valid:   bool array of shape () indicating whether std > eps (False → this group carries no preference)
        """
        assert mpjpe.shape == pampjpe.shape == pve.shape
        a1, a2, a3 = alpha

        # 1) scalar reward: lower error → higher reward
        r = -(a1 * mpjpe + a2 * pampjpe + a3 * pve)

        # 2) group-wise z-score → advantage
        mean_r = r.mean()
        std_r = r.std()
        valid = std_r > eps
        if valid:
            ...
            # Degenerate: no variance in rewards → no preference signal in this group
            A = np.zeros_like(r)
