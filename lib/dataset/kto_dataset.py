...
from dataset.pw3d_dpo import PW3D_DPO, PW3D_KTO
...
class KTODataset(data.Dataset):
    ...
    def __init__(self,
        ...
        if cfg.training.get('dpo', False) or cfg.training.get('kto', False):                  # for DPO training set length control
            dpo_set_list = []
            for i, partition in enumerate(cfg.dataset.partition):
                if partition > 0:
            ...
            logging.info(f"DPO Dataset Total Size: {self.tot_size}")
